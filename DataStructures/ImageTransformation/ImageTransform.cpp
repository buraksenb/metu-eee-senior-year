...
PNG createSpotlight(PNG image, int centerX, int centerY) {
	for (unsigned x = 0; x < image.width(); x++) {
        for (unsigned y = 0; y < image.height(); y++) {
            ...
            int dist_x = abs(static_cast<int>(x) - centerX);
            int dist_y = abs(static_cast<int>(y) - centerY);
...
PNG watermark(PNG firstImage, PNG secondImage) {
    ...
    const int min_width = firstImage.width() < secondImage.width() ? firstImage.width() : secondImage.width(); 
    const int min_height = firstImage.height() < secondImage.height() ? firstImage.height() : secondImage.height();
